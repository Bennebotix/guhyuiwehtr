const INITIAL_PIXEL_SIZE = 10000000;
const COLUMNS = 40;
const ROWS = 28;
const SPEED = 1;
...
function drawAndUpdate(time) {
  ...
  for (var i = 1; i < 3; i++) {
    if (i == 1) {
      gameCtx.beginPath();
      gameCtx.moveTo(borderWidth, borderWidth);
      gameCtx.lineTo(borderWidth, borderWidth + (pixelSize * ROWS));
      gameCtx.stroke();
    } else {
      gameCtx.beginPath();
      gameCtx.moveTo(borderWidth + (pixelSize * COLUMNS), borderWidth);
      gameCtx.lineTo(borderWidth + (pixelSize * COLUMNS), borderWidth + (pixelSize * ROWS));
      gameCtx.stroke();
    }
  ...
  for (var j = 1; j < 3; j++) {
    if (j == 1) {
      gameCtx.beginPath();
      gameCtx.moveTo(borderWidth, borderWidth);
      gameCtx.lineTo(borderWidth + (pixelSize * COLUMNS), borderWidth);
      gameCtx.stroke();
    } else {
      gameCtx.beginPath();
      gameCtx.moveTo(borderWidth, borderWidth + (pixelSize * ROWS));
      gameCtx.lineTo(borderWidth + (pixelSize * COLUMNS), borderWidth + (pixelSize * ROWS));
      gameCtx.stroke();
    }
  }
  ...
  // Draw score
  const score = snakeSize - INITIAL_SNAKE_SIZE - 1 + start;
